...
.NOTES
    ...
    Created: June 4, 2026
    Version: 6.0.0 - Okta Integration & MSI Enhancement Edition
...
$enginePath = Join-Path $script:ToolRoot "src"

# List of engines to load (25 total engines)
$enginesToLoad = @(
    # Core Engines (3)
    @{ Name = "PathEngine"; Path = "Core\PathEngine.psm1" },
    @{ Name = "WorkflowEngine"; Path = "Core\WorkflowEngine.psm1" },
    @{ Name = "TestOrchestrationEngine"; Path = "Core\TestOrchestrationEngine.psm1" },
    
    # Packaging Engines (8)
    @{ Name = "MetadataEngine"; Path = "Packaging\MetadataEngine.psm1" },
    @{ Name = "DetectionEngine"; Path = "Packaging\DetectionEngine.psm1" },
    @{ Name = "SwitchEngine"; Path = "Packaging\SwitchEngine.psm1" },
    @{ Name = "UninstallEngine"; Path = "Packaging\UninstallEngine.psm1" },
    @{ Name = "ValidationEngine"; Path = "Packaging\ValidationEngine.psm1" },
    @{ Name = "FolderEngine"; Path = "Packaging\FolderEngine.psm1" },
    @{ Name = "ProcessEngine"; Path = "Packaging\ProcessEngine.psm1" },
    @{ Name = "BuildEngine"; Path = "Packaging\BuildEngine.psm1" },
    
    # Testing Engines (3)
    @{ Name = "InstallTestEngine"; Path = "Testing\InstallTestEngine.psm1" },
    @{ Name = "ScanEngine"; Path = "Testing\ScanEngine.psm1" },
    @{ Name = "UninstallScanEngine"; Path = "Testing\UninstallScanEngine.psm1" },
    
    # Documentation Engines (5)
    @{ Name = "DocumentationOrchestrator"; Path = "Documentation\DocumentationOrchestrator.psm1" },
    @{ Name = "CaptureEngine"; Path = "Documentation\CaptureEngine.psm1" },
    @{ Name = "DocumentGeneratorEngine"; Path = "Documentation\DocumentGeneratorEngine.psm1" },
    @{ Name = "InstallationDetectorEngine"; Path = "Documentation\InstallationDetectorEngine.psm1" },
    @{ Name = "SnagitController"; Path = "Documentation\SnagitController.psm1" },
    
    # Deployment Engines (2)
    @{ Name = "DeploymentEngine"; Path = "Deployment\DeploymentEngine.psm1" },
    @{ Name = "GitLabAuthEngine"; Path = "Deployment\GitLabAuthEngine.psm1" },
    
    # Utility Engines (4)
    @{ Name = "AuthenticationModule"; Path = "Utilities\AuthenticationModule.psm1" },
    @{ Name = "CustomCommandsEngine"; Path = "Utilities\CustomCommandsEngine.psm1" },
    @{ Name = "StartupFileManager"; Path = "Utilities\StartupFileManager.psm1" },
    @{ Name = "WebSearchEngine"; Path = "Utilities\WebSearchEngine.psm1" }
)
...
#region Step 2: Master Template Check/Download - DISABLED
# GitLab auto-download removed - Master Template included in application package
# Users will have Master Template available immediately without download
...
#endregion First-Time Setup and Master Template
#>

#endregion Step 2 Check
...
function New-PackagingFolder {
    ...
    try {
        ...
        # Check if any custom commands were entered
        foreach ($key in $commandSections.Keys) {
            if (-not [string]::IsNullOrWhiteSpace($commandSections[$key])) {
        ...
        if ($hasCustomCommands) {
            Update-Status "Inserting custom commands into Startup.pss..." "Blue"
            $form.Refresh()
            ...
            try {
                ...
                if ($insertResult.Success) {
                    if ($insertResult.InsertedSections.Count -gt 0) {
                        Update-Status "Custom commands inserted: $($insertResult.InsertedSections.Count) section(s)" "Green"
                    }
                } else {
                    Write-Warning "Custom commands insertion failed: $($insertResult.Message)"
                }
            }
            catch {
                Write-Warning "Error inserting custom commands: $($_.Exception.Message)"
            }
        }
...
$form.Text = "FRB Packaging Tool v6.0.0"

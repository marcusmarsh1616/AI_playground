...
<#
.SYNOPSIS
    Validation Document Generator Module
...
# Module variables
$script:TemplatePath = "$PSScriptRoot\..\templates\Professional-Validation-Template.html"
...
function New-ValidationDocument {
    ...
    param(
        ...
        [Parameter()]
        [string]$OSVersion = "Windows 10 x64",
...
"@
    ...
    if ($markerIndex -ge 0) {
        ...
        if ($divStart -ge 0) {
            ...
            $firstClosing = $afterMarker.IndexOf('</div>')
            
            if ($firstClosing -ge 0) {
                # Find the second </div> (closes the outer div)
                $afterFirst = $afterMarker.Substring($firstClosing + 6)
                $secondClosing = $afterFirst.IndexOf('</div>')
                
                if ($secondClosing -ge 0) {
                    $endPos = $markerIndex + $firstClosing + 6 + $secondClosing + 6
                    ...
                    Write-Warning "Could not find closing tags for Figure $FigureNumber"
                }
            }
        }
    } else {
...
function Set-ValidationDetails {
    ...
    $HtmlContent = $HtmlContent.Replace('<td>Desktop Shortcuts:</td><td>None</td>', "<td>Desktop Shortcuts:</td><td>$DesktopShortcuts</td>")

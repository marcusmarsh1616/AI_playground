...
<#
	...
	===========================================================================
	 ...
	 Organization: 	NIT
	 Filename:     	{RITM}_{APP_NAME}_Detect_R1.ps1
	===========================================================================
	.DESCRIPTION
		...
		Target Version: {TARGET_VERSION} (minimum required)
		Vendor: {VENDOR_NAME}
		RITM: {RITM}
		
	.PARAMETER Test
		Run in test mode - outputs what was found for validation
		Usage: .\{RITM}_{APP_NAME}_Detect_R1.ps1 -Test
#>
#region [DO NOT TOUCH]
[CmdletBinding()]
param
...
$infoHash = @{
	'{APPLICATION_DISPLAY_NAME}' = '{TARGET_VERSION}'
}
...
if ($Test) {
	...
	if ($detected -and $detected.Count -gt 0) {
		...
		Write-Host "[NOT FOUND] No applications matching pattern '{APPLICATION_DISPLAY_NAME}'" -ForegroundColor Red
		Write-Host "" -ForegroundColor Cyan
		Write-Host "[DETECTION RESULT] Would return: NOT DETECTED (exit 1)" -ForegroundColor Red
		Write-Host "" -ForegroundColor Cyan

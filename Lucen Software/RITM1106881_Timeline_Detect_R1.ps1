...
<#
	...
	===========================================================================
	 ...
	 Organization: 	NIT
	 Filename:     	RITM1106881_Timeline_Detect_R1.ps1
	===========================================================================
	.DESCRIPTION
		...
		Application: ^Lucen Timeline(?:\s|$)
		Target Version: 14.19.0 (minimum required)
		Vendor: Lucen Software
		RITM: RITM1106881

	.PARAMETER Test
		Run in test mode - outputs what was found for validation
		Usage: .\RITM1106881_Timeline_Detect_R1.ps1 -Test
#>
#region [DO NOT TOUCH]
[CmdletBinding()]
param
...
# Global variable for test results (PS 5.1 compatibility - scriptblocks can't reliably access $script: scope)
if (-not (Test-Path variable:global:testResults)) {
    $global:testResults = @()
}
...
function Get-InstalledVersion{
	...
	# Reset test results for this invocation
	$global:testResults = @()

	$resultFinal = New-Object System.Collections.ArrayList
	...
	[scriptblock]$DetectDefault = {
		...
		# Use passed parameters instead of parent scope variables
		...
		if ($ContextParam -eq 'User') { $regPaths = @("Registry::HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall","Registry::HKCU\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall") }

		# Standard Uninstall registry paths
		...
		# ADDITIONAL: Windows Installer UserData location (per-user MSI installations)
		# These apps don't appear in standard Uninstall keys but are visible in Programs and Features
		# Example: Lucen Timeline and other user-context MSI packages
		if ($ContextParam -eq 'User') {
			try {
				$userDataPath = "Registry::HKLM\Software\Microsoft\Windows\CurrentVersion\Installer\UserData"

				# Enumerate all user SIDs
				$userSids = Get-ChildItem -Path $userDataPath -ErrorAction SilentlyContinue | Where-Object { $_.PSChildName -match '^S-1-5-21' }

				foreach ($sidKey in $userSids) {
					$productsPath = Join-Path $sidKey.PSPath "Products"

					if (Test-Path $productsPath) {
						# Enumerate all product GUIDs under this user
						$products = Get-ChildItem -Path $productsPath -ErrorAction SilentlyContinue

						foreach ($product in $products) {
							$installPropsPath = Join-Path $product.PSPath "InstallProperties"

							if (Test-Path $installPropsPath) {
								try {
									$installProps = Get-ItemProperty -Path $installPropsPath -ErrorAction SilentlyContinue

									# Check if DisplayName matches the pattern
									if ($installProps.DisplayName -match $displayName) {
										try {
											[version]$installedVersion = $installProps.DisplayVersion
											...
											if ($DetectTypeParam -eq 'Test') {
												$newResult = [PSCustomObject]@{
													...
													MeetsRequirement = ($installedVersion -ge $displayVersion)
													Comparison = if ($installedVersion -ge $displayVersion) { "PASS (>=)" } else { "FAIL (<)" }
													RegistryPath = "$installPropsPath (Windows Installer UserData)"
												}
												$currentResults = $global:testResults
												$currentResults += $newResult
												Set-Variable -Name testResults -Value $currentResults -Scope Global
											}
											...
											if ($DetectTypeParam -eq 'Test') {
												$global:testResults += [PSCustomObject]@{
													DisplayName = $installProps.DisplayName
													InstalledVersion = "ERROR: $($installProps.DisplayVersion)"
													TargetVersion = $displayVersion.ToString()
													MeetsRequirement = $false
													Comparison = "ERROR (version parse failed)"
													RegistryPath = "$installPropsPath (Windows Installer UserData)"
									...
									# Silent failure for individual product read errors
									if ($DetectTypeParam -eq 'Test') {
										Write-Verbose "Error reading InstallProperties: $installPropsPath - $_"
									}
								}
							}
						}
				...
				# Silent failure for Windows Installer UserData enumeration errors
				if ($DetectTypeParam -eq 'Test') {
					Write-Verbose "Error enumerating Windows Installer UserData: $_"
				}
			}
		}
	}
	...
	$SearchArray.Keys | ForEach-Object {
		...
		switch($DetectType) {
			'File' {& $DetectFile -FilePath $fPath -FileVersion $fVersion -DetectTypeParam $DetectType}
			default {& $DetectDefault -DisplayName $dName -DisplayVersion $dVersion -ContextParam $Context -DetectTypeParam $DetectType}
		}
...
$installContext = 'User' # Determines which registry location to search. (i.e. "System = HKLM:" vs. "User = HKCU:")

# Application details
$appName = '^Lucen Timeline(?:\s|$)'
$appVersion = '14.19.0'
...
if ($Test) {
	...
	# Handle single-result unwrapping in PowerShell (single array items get unwrapped to objects)
	if ($detected) {
		$detectedArray = @($detected)  # Force to array
		Write-Host "[FOUND] $($detectedArray.Count) application(s) found:" -ForegroundColor Green
		Write-Host "" -ForegroundColor Cyan

		$detectedArray | ForEach-Object {
		...
		$passCount = @($detectedArray | Where-Object { $_.MeetsRequirement }).Count
		$failCount = @($detectedArray | Where-Object { -not $_.MeetsRequirement }).Count

		Write-Host "Summary:" -ForegroundColor Cyan
		Write-Host "  Total Found:   $($detectedArray.Count)" -ForegroundColor White

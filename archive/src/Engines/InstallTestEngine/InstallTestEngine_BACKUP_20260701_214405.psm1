...
function Start-UninstallationTest {
    ...
    try {
        ...
        if ($uninstallProcess) {
            ...
            if ($leftovers.Found) {
                $leftoverParts = @(
                    "Leftover items detected after uninstallation:",
                    "",
                    ($leftovers.Summary -join [Environment]::NewLine),
                    "",
                    "Would you like to remove these leftovers now?",
                    "",
                    "Note: This will require administrator elevation."
                )
                $leftoverMsg = $leftoverParts -join [Environment]::NewLine
                ...
                if ($cleanupResponse -eq 'Yes') {
                    ...
                    if ($cleanupResult.Success) {
                        $cleanupParts = @(
                            ...
                            "",
                            "Successfully Removed: $($cleanupResult.TotalCleaned) item(s)"
                        )
                        if ($cleanupResult.TotalFailed -gt 0) {
                            $cleanupParts += "Failed to Remove: $($cleanupResult.TotalFailed) item(s)"
                        }
                        $cleanupMsg = $cleanupParts -join [Environment]::NewLine
                        ...
                        if ($leftovers.Found) {
                            $remainingParts = @(
                                "Warning: Some leftover items could not be removed:",
                                "",
                                ($leftovers.Summary -join [Environment]::NewLine),
                                "",
                                "Manual cleanup may be required."
                            )
                            $remainingMsg = $remainingParts -join [Environment]::NewLine
                            [System.Windows.Forms.MessageBox]::Show(
                                $remainingMsg,
                                "Cleanup Incomplete",
                                [System.Windows.Forms.MessageBoxButtons]::OK,
                            ...
                            $verifiedMsg = @(
                                "All leftover items successfully removed.",
                                "",
                                "Verification scan confirmed clean system."
                        ...
                        $errorMsg = $errorParts -join [Environment]::NewLine
                        [System.Windows.Forms.MessageBox]::Show(
                            $errorMsg,
                            "Cleanup Error",
                            [System.Windows.Forms.MessageBoxButtons]::OK,

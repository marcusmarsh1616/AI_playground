...
function Copy-PackageToNetworkShare {
    ...
    .PARAMETER Edition
        Product edition (optional, for folder structure)
    .PARAMETER Version
    ...
    .EXAMPLE
        $result = Copy-PackageToNetworkShare -PackagePath "C:\Packages\Adobe\Reader\Pro\23.1.0" -NetworkSharePath "\\server\packages" -Vendor "Adobe" -ProductName "Reader" -Edition "Pro" -Version "23.1.0"
    .OUTPUTS
    ...
    try {
        ...
        # Build target path mirroring local folder structure: Vendor\ProductName\Edition\Version
        ...
        if (-not [string]::IsNullOrWhiteSpace($Edition)) {
            $targetPath = Join-Path $targetPath $Edition
        }
